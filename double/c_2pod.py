...
def inverse_c_2pod(x):
    return (x + 30.143)/0.332


def c_2pod(x):
    return 0.332*x - 30.143

if __name__ == '__main__':
    mean_vh = np.load('./snap/mean_vh.npy')
    wsped = np.load('./snap/ten_wind.npy')
    wsped_ = inverse_c_2pod(mean_vh)
    ...
    plt.plot(x, y)
    plt.title('wind and sigma0 relation c_2pod')
    plt.xlabel('wind speed')
    plt.ylabel('sigma0_vh')
    plt.tick_params(top='off', right='off')
    plt.savefig('correlation_c_2pod.png')
